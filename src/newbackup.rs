...
use std::{fs::{self, read_dir}, process::Command, time::SystemTime, ffi::OsStr, os::unix::prelude::OsStrExt};
...
impl Game {
    pub fn backup(&self, backup_location: &str) {
        ...
        if let Some(v) = &self.file_path {
            ...
            let file_name = format!("{name}-{}", &DateTime::<Utc>::from(SystemTime::now()).to_rfc3339());
            let _ = Command::new("tar")
                .args([
                      "--create",
                      "--gzip",
                      &format!("--listed-incremental={backup_location}/{name}/{name}.sngz"),
                      &format!("--file={backup_location}/{name}/{}", &file_name[..file_name.len() - 6]),
                      &v
                ])
                .spawn();
        }
